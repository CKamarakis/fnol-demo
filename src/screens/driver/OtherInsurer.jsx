...
export function scrOtherInsurer() {
  ...
  const body = (
    <div>
      ...
      <p className="lbl">{T('gInsQ')}</p>
      <div className="grid2" style={{ marginBottom: '14px' }}>
        <Choice act="set-other-insured" value="yes" label="Yes"
          selected={d.otherInsured === true} />
        <Choice act="set-other-insured" value="no" label={T('gInsNotSure')}
          selected={d.otherInsured === false} />
      </div>

      {textField(T('gInsName'), 'otherInsurer', 'HUK-Coburg, Allianz…')}
      {textField(T('gInsPolicy'), 'otherPolicy', '')}
    </div>
  );
